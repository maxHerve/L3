 ...
 */
int remove_element(list_elem_t ** ppl, int value) {
	list_elem_t* previousElem = NULL;
	list_elem_t* currentElem = *ppl;

	while(currentElem != NULL){
		if(currentElem->value == value){
			if(previousElem == NULL){
				*ppl = currentElem->next;
			}
			else if(currentElem->next == NULL){
				previousElem->next = NULL;
			}
			else{
				previousElem->next = currentElem->next;
			}
			free_element(currentElem);
			currentElem = NULL;
			return 0;
		}
...
void reverse_list(list_elem_t * * l) {
	list_elem_t* previousElem = NULL;
	list_elem_t* currentElem = *l;
	list_elem_t* nextElem = NULL;
